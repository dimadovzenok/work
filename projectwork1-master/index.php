...
<h1>Otsige kõiki nimed, mis algavad tähega Y.</h1>
<ul>
    <?php
    foreach ($func->project as $arua){
        if (substr(strtolower($arua->kasutaja->nimi),0,1)=="y"){
            echo "<li>". ($arua->kasutaja->nimi) . "</li>";

        }
    }
    ?>
</ul>
<h2>Koik aeg kes sisesta poze 10.11.2021</h2>
<ul>
    <?php
    foreach ($func->project as $arua){
        if ($arua->siselogimiseaeg > 10.112021 ){
            echo "<li>". ($arua->kasutaja->nimi) . ",";
            echo ($arua->siselogimiseaeg) . "</li>";
